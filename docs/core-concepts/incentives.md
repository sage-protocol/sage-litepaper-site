...
Both paths are governed through the DAO Timelock. The **CLI** supports `create`, `fund`, `set-root`, `status`, `claim`, and `finalize`.
...
-   **Pay-to-Pin Credits**: A 402-based credit system allows users to pay for pinning services directly through the CLI. Users can buy credits and spend them to pin CIDs via the managed worker, with revenue routed to DAO and protocol treasuries.
